...
<main class="app-container {activeStep === 4 ? 'board-mode' : 'full-width'}">
    <div class="controls-section">
        ...
        {#if activeStep === 1}
            <div class="step-content">
                <label>
                    <input type="checkbox" checked={selectedPlatform === 'Chesscom'} 
                        on:change={() => handlePlatformChange('Chesscom')} />
                    Chesscom
                </label>
                <label>
                    <input type="checkbox" checked={selectedPlatform === 'Lichess'} 
        ...
        {#if activeStep === 3}
            <div class="step-content threshold-column">
                {#each thresholdValues as value, index}
                    <label>
                        Threshold {index + 1}:
                        <input type="number" step="0.01" bind:value={thresholdValues[index]} />
                    </label>
                {/each}
            </div>
        {/if}
        ...
        <div class="navigation-buttons">
            <button on:click={goToPreviousStep} disabled={activeStep === 0}>Previous</button>

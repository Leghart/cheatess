...
<div class="command-interface">
    <div class="command-container">
        {#each commands as command}
            <p><strong>Action:</strong> {command.action}, <strong>Parameters:</strong> {command.parameters.join(', ')}</p>
        {/each}
    </div>
    <input type="text" bind:value={action} placeholder="Action" />
    <input type="text" bind:value={parameters} placeholder="Parameters (comma separated)" />

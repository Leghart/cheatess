...
class ScanningView(ctk.CTkFrame):
    def __init__(self, master: TabView, engine_handler: Engine):
        ...
        self.thread_update_evaluation = QueueThread(
            ...
            redirect_data=self._update_evalbar,
        ).start()

        self.evalbar_label = ctk.CTkLabel(self.tab, text="0.0")
        self.evalbar_label.grid(row=1, column=0)
        self.evalbar = ctk.CTkProgressBar(
            self.slider_progressbar_frame, orientation="horizontal"
        )
...
class StockfishView(ctk.CTkFrame):
    ...
    def _set_level(self):
        ...
        self.elo_label = ctk.CTkLabel(
            self.tab, text=f"ELO level: {self._level.get()}", anchor="w"
        )
        ...
        self.level_slider = ctk.CTkSlider(
            ...
            command=lambda _: self.elo_label.configure(
                text=f"ELO level: {self._level.get()}"
            ),
        )
        self.level_slider.grid(
            row=6, column=0, columnspan=2, padx=(20, 10), pady=(10, 10), sticky="ew"
        )

    def _set_path_to_stockfish_engine(self):
        label = ctk.CTkLabel(
            self.tab, text="Path to downloaded stockfish engine:", anchor="w"
        )
        label.grid(row=7, columnspan=2, padx=20, pady=(20, 0))

        self._engine_path = tk.StringVar(value=self.cache["stockfish_engine_path"])
        self.engine_path_entry = ctk.CTkEntry(
            self.tab, width=500, textvariable=self._engine_path
        )
    ...
    def _set_buttons(self):
        def set_default_values():
            ...
            LogQueue.send(
                Message("Default settings have been restored", LogLevel.WARNING)
            )
        ...
        def save():
            ...
            if not engine_path:
                ...
                LogQueue.send(
                    Message("Typed path to engine doesn't exist", LogLevel.ERROR)
                )
                return
        ...
        to_default_button = ctk.CTkButton(
            self.tab, text="Reset settings", command=set_default_values
        )
        to_default_button.grid(
            row=9, column=0, padx=(20, 10), pady=(10, 10), sticky="ew"
        )
...
class GeneralSettingsView(ctk.CTkFrame):
    ...
    def __init__(self, master: ctk.CTkTabview):
        ...
        appearance_mode_label = ctk.CTkLabel(
            self.master.tab("General"), text="Appearance Mode:", anchor="w"
        )
        ...
        self.hide_help_button = ctk.CTkButton(
            self.master.tab("General"), text="Show help", command=self.__hide_help
        )

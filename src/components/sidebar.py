...
class SideBar(ctk.CTkFrame):
    ...
    def __init__(self, master: App):
        ...
        self.ping = ctk.CTkButton(
            self,
            text="Ping",
            command=self.send_to_rust,
            fg_color="blue",
        )
        self.ping.grid(row=2, column=2, padx=20, pady=10)
        self.ping.configure(
            state="" if self.engine_handler.is_loaded_coords else "disabled"
        )

        self.scan_screen_button = ctk.CTkButton(
            self, text="Scan board", command=self._create_screen_canvas
        )
        ...
        self.start_scanning_button = ctk.CTkButton(
            self,
            text="Start scanning",
            command=self.engine_handler.start_scaning,
            fg_color="green",
        )
        self.start_scanning_button.grid(row=2, column=0, padx=20, pady=10)
        self.start_scanning_button.configure(
            state="" if self.engine_handler.is_loaded_coords else "disabled"
        )
        ...
        self.stop_scanning_button.configure(
            state="normal" if self.engine_handler.is_loaded_coords else "disabled"
        )
        ...
        self.clear_logs_button = ctk.CTkButton(
            self,
            text="Clear logs",
            command=self.logbox.clear_logs,
            text_color="black",
            fg_color="white",
        )
        self.clear_logs_button.grid(
            row=6, column=0, columnspan=2, sticky="nsew", padx=20, pady=10
        )
    ...
    def _create_screen_canvas(self):
        ...
        self.snip_surface = ctk.CTkCanvas(
    ...
    def __on_snip_drag(self, event):
        ...
        self.snip_surface.coords(
            1,
            self.snippet.start_x,
            self.snippet.start_y,
            self.snippet.current_x,
            self.snippet.current_y,
        )
    ...
    def __on_button_release(self, event):
        ...
        print(self.engine_handler.board_coords)
        return event
    ...
    def send_to_rust(self):
        message = "dupa"
        data = {"cmd": "Ping", "message": message, "value": 42}
